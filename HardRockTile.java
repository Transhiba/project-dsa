...
public class HardRockTile extends Tile {
	...
	public boolean hurt(Level level, int x, int y, Mob source, int dmg, Direction attackDir) {
		hurt(level, x, y, /*0 */  dmg); // dmg là do mị thêm vô
		return true;
	}
// đập đá lâu quá nên ms lm v
	...
	*/
	...
	@Override
	public boolean interact(Level level, int xt, int yt, Player player, Item item, Direction attackDir) {
		// DEMO MODE: đập luôn không cần tool
		int fakeDamage = 5; // mỗi lần đập trừ 5 máu
		hurt(level, xt, yt, fakeDamage);
...
// hết phần thêm vô
	public void hurt(Level level, int x, int y, int dmg) {
		...
		int hrHealth = 11;// chinhr từ 200 xuống 11
